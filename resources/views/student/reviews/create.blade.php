                ...
                <form method="POST" action="{{ route('student.reviews.store') }}">
                    ...
                    <div class="mb-4">
                        ...
                        <div class="flex items-center space-x-2" id="star-rating">
                            @for($i=1; $i<=5; $i++)
                                <svg data-value="{{ $i }}" class="w-8 h-8 cursor-pointer star {{ old('rating', 5) >= $i ? 'text-yellow-400' : 'text-gray-300' }}" fill="currentColor" viewBox="0 0 20 20">
                    ...
                    <div class="mb-4">
                        ...
                        <textarea name="review" class="w-full border rounded-lg px-3 py-2" rows="4" required>{{ old('review') }}</textarea>
                        @error('review')<div class="text-red-500 text-xs mt-1">{{ $message }}</div>@enderror
                    </div>
                    <div class="flex justify-end">
                        <button type="submit" class="px-6 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700">Kirim</button>
